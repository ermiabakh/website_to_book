...
# Updated TqdmToQueue class: pass and use the proper event loop.
...
class Crawler:
    def __init__(self, root_url: str, max_depth: int = 3):
        ...
        self.base_path = urlparse(root_url).path  # Save the base path of the user-provided URL
        if not self.base_path.endswith('/'):
            self.base_path += '/' # Ensure base path ends with slash for correct prefix matching
    ...
    def is_valid_url(self, url: str) -> bool:
        ...
        if parsed.netloc != self.base_domain:
            return False
        # Only allow URLs whose path starts with the user provided base path
    ...
    async def crawl(self) -> Tuple[List[str], List[str]]:
        ...
        async with async_playwright() as p:
            ...
            tasks = set()
            # Pass the proper event loop to TqdmToQueue
...
async def generate_pdf(task: Tuple[int, str, Path, asyncio.Queue]) -> Tuple[int, str, bool]:
    ...
    try:
        ...
        await page.pdf(
            path=str(pdf_path),
            format='A3',  # Updated to A3 size
            print_background=True,
            margin={'top': '5mm', 'right': '5mm', # Reduced padding
                    'bottom': '5mm', 'left': '5mm'}  # Reduced padding
        )
...
def merge_pdfs(pdf_files: List[Tuple[int, str]], output_path: str, temp_dir: Path):
    ...
    # For synchronous functions, use asyncio.get_event_loop() here if the event loop is still running.
    with TqdmToQueue(total=len(chunks), desc="Merging PDF chunks", unit="chunk", loop=asyncio.get_event_loop()) as pbar:
        results = []
        for i, result in enumerate(multiprocessing.Pool(processes=num_processes).imap_unordered(
                merge_chunk, [(chunk, temp_dir_str, i) for i, chunk in enumerate(chunks)])):
    ...
    with TqdmToQueue(total=len(results), desc="Merging Chunks to Final PDF", unit="chunk", loop=asyncio.get_event_loop()) as pbar:
...
async def run_conversion(url: str, max_depth: int, workers: int, output_path: str):
    ...
    try:
        ...
        async with async_playwright() as p:
            ...
            with TqdmToQueue(total=len(tasks), desc="Generating PDFs", unit="page", loop=asyncio.get_event_loop()) as pbar:
                results = []
...
async def download():
    ...
    async def file_stream():
        return await send_file(file_path, mimetype='application/octet-stream', as_attachment=True, download_name=filename)

    return await download_file_via_stream(filename)

async def download_file_via_stream(filename):
    file_path = os.path.join(OUTPUT_DIR, filename)
    if not os.path.exists(file_path):
        return jsonify({"status": "error", "message": "File not found"}), 404
