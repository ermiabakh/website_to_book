...
class Crawler:
    ...
    async def crawl(self) -> List[str]:
        ...
        async with async_playwright() as p:
            ...
            page_pool: List[Page] = [await context.new_page() for _ in range(page_pool_size)] # type: ignore
    ...
    async def crawl_page(self, url: str, depth: int, page: Page): # Type hint for page
        try:
            # Changed from 'networkidle' to 'domcontentloaded' for potentially faster initial load.
            # You may need to revert to 'networkidle' if 'domcontentloaded' is not sufficient for all sites.
            await page.goto(url, timeout=120000, wait_until='domcontentloaded')

            # Removed fixed sleep and replaced with intelligent wait for content to render.
            # **Important:** Adjust the selector '#main-content' to a suitable CSS selector that indicates
            # when the main content of the page has loaded *after* Javascript execution.
            # Inspect your target website to find the best selector. Examples: '#content', '#main', '.loaded-content', 'footer'.
            try:
                await page.wait_for_selector('#main-content', timeout=30000) # Wait for main content to load (adjust selector!)
            except:
                logging.warning(f"Selector '#main-content' not found on {url} within timeout, proceeding anyway. Content might not be fully rendered.")
...
async def generate_pdf(task: Tuple[int, str, Path, asyncio.Queue]) -> Tuple[int, str, bool]:
    ...
    try:
        page: Page = await page_queue.get() # Type hint for page from queue
        # Changed from 'networkidle' to 'domcontentloaded' for potentially faster initial load.
        # You may need to revert to 'networkidle' if 'domcontentloaded' is not sufficient for all sites.
        await page.goto(url, timeout=120000, wait_until='domcontentloaded')

        # Removed fixed sleep and replaced with intelligent wait for content to render.
        # **Important:** Use the *same selector* as in `crawl_page` ('#main-content' in this example, but adjust!).
        try:
            await page.wait_for_selector('#main-content', timeout=30000) # Wait for main content to load (same selector as in crawl_page!)
        except:
            logging.warning(f"Selector '#main-content' not found on {url} for PDF generation within timeout, proceeding anyway. PDF might not be complete.")
